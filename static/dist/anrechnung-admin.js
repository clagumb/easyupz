export async function init() {
    ...
    try {
        ...
        const anrechnungen = await response.json();
        const tbody = document.querySelector("#anrechnungen-tabelle tbody");
        const template = document.getElementById("anrechnung-zeile");
        if (!tbody || !template)
            return;
        tbody.innerHTML = "";
        for (const anrechnung of anrechnungen) {
            const clone = template.content.cloneNode(true);
            clone.querySelector(".kurzform").textContent =
                anrechnung.kurzform;
            clone.querySelector(".anzeigeform").textContent =
                anrechnung.anzeigeform;
            clone.querySelector(".langform").textContent =
                anrechnung.langform;
...
function createEingabezeile() {
    ...
    function checkInputs() {
        const allFilled = kurzformInput.value.trim() &&
